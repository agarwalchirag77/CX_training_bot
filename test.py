...
datafile_path = "testquery.txt"

app = FastAPI()
load_dotenv()


intercom_admin_id=6500280
intercom_assignee_id=6500280
tag_id = 8394964
...
async def intercom(background_tasks: BackgroundTasks, data: dict = Body()):
    tags = data.get("data").get("item").get("tags").get("tags")
    for tag in tags:
        print(f"Request: {tag.get('id')}, {str(tag_id)}")
        #print(str(tag.get("id")) == str(tag_id))
